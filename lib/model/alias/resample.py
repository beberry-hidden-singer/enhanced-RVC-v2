...
"""from https://github.com/PlayVoice/NSF-BigVGAN/blob/main/model/alias/resample.py"""

import torch.nn as nn
from torch.nn import functional as F

from model.alias.filter import LowPassFilter1d
from model.alias.filter import kaiser_sinc_filter1d
    ...
    def __init__(self, ratio=2, kernel_size=None, C=None):
        ...
        if C is not None:
            self.conv_transpose1d_block = nn.ModuleList([nn.ConvTranspose1d(C, C,
                                                                            ...
                                                                            groups=C,
                                                                            bias=False)])
            self.conv_transpose1d_block[0].weight = nn.Parameter(filter.expand(C, -1, -1).clone())
            # self.conv_transpose1d_block[0].weight = nn.Parameter(self.filter.repeat_interleave(C,0).clone())
